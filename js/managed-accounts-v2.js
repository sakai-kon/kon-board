...
// Must exactly match the synthetic email format used by admin-moderation and managed-login.js.
...
function addManagedLoginLink(){const host=app?.querySelector('.auth-actions');if(!host||host.querySelector('#managed-login-link'))return;const a=document.createElement('a');a.id='managed-login-link';a.className='managed-login-link';a.href='./managed-login.html';a.textContent='管理者発行アカウント専用ログインはこちら';host.appendChild(a);}
