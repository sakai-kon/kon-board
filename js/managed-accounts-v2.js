...
export function handleManagedRoute(){const p=location.hash.slice(1).split('?')[0]||'/home';if(p==='/managed-login')return renderManagedLogin();if(p==='/admin/accounts')return renderManagedAccounts();if(p==='/login'||p==='/auth')addManagedLoginLink();}
window.addEventListener('hashchange',handleManagedRoute);handleManagedRoute();
